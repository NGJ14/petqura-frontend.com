...
const ProductDetails = () => {
  ...
  const handlePincode = () => {
    dispatch(checkPincode({ pin: { pincode: pin } }));
  };
...
:null                          } */}
                ...
                {console.log(ShopDetails?.userProductDetails?.related_products)}
                <h3 className="mb-5 ml-3 pl-1">Related Products</h3>
                <div className=" common-container products">
                  <div className="row multi-row-clearfix ">
                    {ShopDetails?.userProduct?.products?.length
                      ? ShopDetails?.userProduct?.products
                          ?.slice(0.6)
                          ?.map((product, i) =>
